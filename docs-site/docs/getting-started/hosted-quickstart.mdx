...
Image requests use the same router model-group names. The router filters image-bearing requests to targets that have passed vision validation and advertise `image` in `input_modalities`.
...
router_model = os.environ.get("ROUTER_MODEL", "default")
