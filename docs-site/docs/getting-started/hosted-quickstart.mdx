...
Use the endpoint for your self-managed deployment with a router-issued caller
token. The token authenticates the caller to Metrum Smart Router; provider keys
stay server-side.

If you do not have a running router yet, start with
[Local Quickstart](./local-quickstart). From Getting Started, you might be
looking for CLI guides: see [Codex CLI](./codex-cli) or
