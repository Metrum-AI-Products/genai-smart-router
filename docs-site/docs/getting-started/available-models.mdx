...
Administrators control this list with each caller token's `allow` entries. Use [User Key Generation](../operations/key-generation) for token creation and [Router Configuration](../configuration/router-config#caller-tokens-and-allow-lists) for allow-list behavior.
