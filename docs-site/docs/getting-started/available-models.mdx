...
The returned IDs are router model groups, not a full inventory of every upstream provider model. A deployment might expose groups such as `default`, `fast`, `coding`, `vision`, or different names chosen by the platform team. Metrum Router does not require any specific group names.
