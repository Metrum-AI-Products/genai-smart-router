...
`/v1/models` remains the caller-facing source of allowed groups. `/v1/codex/models.json` is the Codex-compatible projection of that same allow list. A `401` or `403` means the caller token cannot fetch the catalog; a missing group means that token cannot request it.
