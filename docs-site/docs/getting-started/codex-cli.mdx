...
Codex CLI supports image attachments with `--image`. Use the same deployment-defined coding model group for mixed code and image tasks. In these examples the coding group is `big-coder`; your deployment may use a different group name. The router receives image attachments as OpenAI Responses image inputs and filters that request to multimodal tool-capable upstream targets inside the group.
