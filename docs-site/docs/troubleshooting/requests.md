...
When the bridge is selected, request evidence should show `inbound_dialect` as `openai-chat`, target/attempt dialect as `openai-responses`, and a translation-shape row for `/v1/responses`. Stateful session validation should show a first upstream request without `previous_response_id` and a second same-session upstream request with the prior Responses `id`; operational traces use safe event names such as `bridge_session_requested` and `bridge_session_previous_response_applied`. Use translation field events for safe field names and actions only; they intentionally do not store prompts, tool schemas, tool outputs, images, session header values, bearer tokens, token hashes, provider keys, or full config.
