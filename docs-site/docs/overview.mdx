...
| Debugging requires pasting prompts or keys into tickets | Use request IDs, sanitized upstream errors, scalar diagnostics, and evidence bundles without raw prompts, images, tool outputs, bearer tokens, token hashes, or provider keys by default. |
