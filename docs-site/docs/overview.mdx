...
For a first evaluation, start with the [Commercial Evaluation Path](/docs/evaluation/commercial-evaluation) and choose whether the proof should run on a Metrum-managed hosted endpoint, an enterprise/on-prem licensed package, or a private customer-cloud deployment. Then run one path through the gateway and inspect both the caller result and the operator evidence:
...
[Evaluate GenAI Smart Router](/docs/evaluation/evaluate-smart-router) gives a complete technical checklist and proof points to request. Self-serve credit-card signup and instant public API credits are **not currently offered** as a commercial product; for hosted evaluations, private-cloud deployments, enterprise licenses, and volume-prepurchase access, contact [contact@metrum.ai](mailto:contact@metrum.ai).
...
| Role | Start here |
|---|---|
| Buyer or evaluator | [Evaluation And Case Studies](/docs/evaluation/overview), [Commercial Evaluation Path](/docs/evaluation/commercial-evaluation), [Solution Brief](/docs/solution-brief), [Security And Trust](/docs/evaluation/security-and-trust) |
