...
For bursty coding-agent workloads, operators can enable caller traffic shaping with bounded queueing as a deployment policy. A short `max_wait_ms` and finite `max_depth` let brief bursts wait for token-bucket refill, while oversized bursts still receive `429 traffic-shaped` with `Retry-After`. Queueing is configured per deployment or per caller, and should be validated with normal-request, queued-burst, max-depth rejection, client-cancel, and usage-report smokes before broad rollout.
