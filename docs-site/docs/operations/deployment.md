...
Licensed deployments mount the Metrum-issued `license.json` into the router config directory and enable `server.license`. Keep the license state file in the state directory so renewals, grace state, and clock rollback checks survive restarts. Renewal is replacing the license file and restarting the router or waiting for the configured recheck interval. Private signing keys are never needed by the deployed router.
