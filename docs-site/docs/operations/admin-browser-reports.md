...
CSV export from the browser exports the currently returned table scope: current cursor page for request/security detail, returned top-N rows for aggregate tabs, or visible rows for legacy unpaged responses. Markdown export is a bounded current-filter operational report: it uses the requested `limit` to include the most recent matching request rows and adds an export-scope note when more rows matched the selected window.
