...
The **Operations / Data migrations** view is a read-only, authenticated view of the checked-in migration contract and its safe scalar ledger state. It shows scope, schema/data version, compatibility, effective pending/applied/failed/in-progress state, release, execution/maintenance/lock/timeout class, rollback class, bounded duration, data-job key and state, safe progress totals, and safe error class. A bound data job must be durably validated before its applied schema migration is reported as verified; missing, pending, paused, and cancelled jobs remain pending, running jobs are in progress, and failed jobs remain failed. It deliberately does not expose SQL, DSNs, raw configuration, credentials, tokens, hashes, prompts, images, tool payloads, or database rows.
