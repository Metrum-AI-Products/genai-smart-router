...
The **Operations / Data migrations** view is a read-only, authenticated view of the checked-in migration contract and its safe scalar ledger state. It shows scope, schema/data version, compatibility, pending/applied/failed/in-progress state, release, execution/maintenance/lock/timeout class, rollback class, bounded duration, data-job key, and safe error class. It deliberately does not expose SQL, DSNs, raw configuration, credentials, tokens, hashes, prompts, images, tool payloads, or database rows.
