...
Router caller tokens authenticate applications, users, or evaluation jobs to Metrum Smart Router. Tokens use a readable prefix for traceability plus a random secret suffix. The router stores only token hashes.
