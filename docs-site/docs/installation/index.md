...
|---|---|---|---|---|---|---|---|
| Docker Compose | Deployment-owned reverse proxy or packaged Caddy example; production TLS policy remains customer-owned. | SQLite at `/app/state/usage.sqlite` on the Compose state bind by default; explicit PostgreSQL override for multi-replica or external DB deployments. | Load the packaged image tar, set `SMART_LLMROUTER_VERSION`, run the non-serving SQLite migration gate, then `docker compose up -d` and smoke. | Container logs, optional host Prometheus scrape, and usage DB reports; host log shipping is customer-owned. | One router config governs callers, projects, and provider access; SQLite is one active writer. | Simple durable single-router installations. |
| Linux binary | Customer-owned TLS proxy or service mesh in front of the binary. | Customer-managed SQLite path or explicit PostgreSQL DSN and filesystem state. | Replace binaries under the process supervisor, restore config inputs, restart, then smoke. | Process logs, host log collection, optional Prometheus, and usage DB reports. | One process/config boundary; use separate instances for hard environment or team isolation. | Environments with existing supervisors, hardened host images, database standards, and TLS infrastructure. |
| Kubernetes | Cluster ingress, Gateway API, service mesh, or external load balancer terminates TLS. | One-replica/Recreate SQLite on the state PVC by default; explicit PostgreSQL DSN for multi-replica/external DB deployments. | For a fresh PVC, run the SQLite bootstrap Job, then apply the example overlay and watch rollout. | Pod logs, cluster Prometheus, optional OTel collector, admin reports, and usage DB reports. | Namespace, RBAC, NetworkPolicy, Secrets, and separate router instances for stronger tenant boundaries. | Cluster-native single-router deployments or reviewed PostgreSQL-backed scale-out. |
