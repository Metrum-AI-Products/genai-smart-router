...
Teams own their routing destiny. Metrum Smart Router makes routing policy controllable, measurable, and reversible; it does not guarantee every workload improves automatically. Use evidence to decide whether to promote a routed group, keep a fixed model, split the workload, or collect more data before rollout.

## Proof: same group, different upstream

A routing proof is stronger when the caller-facing group stays fixed while the selected upstream changes with request shape. The repository includes an offline mock proof that posts `testdata/proof/trivial.json` and `testdata/proof/complex.json` through one `dynamic_score` group, then projects `/admin/reports/api/request-evidence`:

```bash
make proof-routing
```

Committed expected output (generated by the mock harness):
...
Illustrative curl against a pre-warmed deployment with decision telemetry and admin drilldown. Cold-start weights and default affinity can pin both requests; treat `make proof-routing` as the reproducible gate:
...
## Prove It, Do Not Feel It: Auditing a Metrum Smart Router Against a Fixed Model
