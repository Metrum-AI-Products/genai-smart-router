...
`migration_policy: deployment-job` is the serving default. Apply and verify migrations with the non-serving `router-migrate` deployment job, then configure `migration_policy: deployment-job` (or `validate`). Every serving policy fails closed unless the ledger is current and compatible and every required bound data job is validated; this includes the synthesized pending state after `apply` and before the first checkpoint. For a reviewed fresh SQLite deployment only, `auto-safe` may validate exactly ordinal `0` of the synthesized pending job after applying checked-in transactional online migrations, and only when that checked-in job is explicitly marked zero-row-safe. Startup opens only when the checkpoint validates with no scanned, updated, skipped, or failed rows. Existing usage rows while validation is still required, or a pre-existing durable `pending`, `running`, `paused`, `cancelled`, `failed`, incompatible, or unrecognized job state, fail closed and remain a non-serving deployment-job responsibility; `auto-safe` never resumes, retries, or batches a job. When the required job is already `validated`, restart skips the zero-row preflight and preserves its durable checkpoint result and counters, including after usage rows accumulate. PostgreSQL production always uses the non-serving procedure.
