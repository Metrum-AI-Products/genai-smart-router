...
For PostgreSQL, supply the migration connection only through the deployment environment (for example, `--dsn-env=ROUTER_USAGE_DB_DSN`); do not place a DSN in a shell command, ticket, or operator output. The non-serving runner verifies the database version, holds a scoped advisory lock, and uses bounded lock and statement timeouts. A reviewed non-transactional maintenance step, such as a concurrent index operation, uses its own explicit maintenance action and a safe backup-evidence reference; its status remains `running`, `failed`, or `applied` until the checked-in postcondition verifies. SQLite is a single-process maintenance deployment: schedule downtime, stop other router processes, take a backup, and confirm integrity and free space before applying work. It is not a multi-replica migration option.
