...
For PostgreSQL, supply the migration connection only through the deployment environment (for example, `--dsn-env=ROUTER_USAGE_DB_DSN`); do not place a DSN in a shell command, ticket, or operator output. Package rollback never runs a reverse migration. The historical usage-validation ledger migration (`2026080501`) is `restore-required`: an older package rejects that unknown applied ledger ID under `validate` or `deployment-job`, so restore the approved pre-migration backup before deploying the older package.
