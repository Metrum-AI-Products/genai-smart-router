...
Metrum Smart Router returns structured errors intended to be useful to both callers and administrators. Every response includes `X-Request-Id`; include that ID when asking an administrator to inspect router traces.
...
| `license-product-mismatch` | 503 | The license is not issued for Metrum Smart Router. | Contact the router operator. | Install the correct product license. |
