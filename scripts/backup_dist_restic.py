...
STABLE_NAMES = {
    ("binary", "amd64"): "smart-llmrouter-linux-amd64.tar.gz",
    ("binary", "arm64"): "smart-llmrouter-linux-arm64.tar.gz",
    ("docker", "amd64"): "smart-llmrouter-docker-linux-amd64.tar.gz",
    ("docker", "arm64"): "smart-llmrouter-docker-linux-arm64.tar.gz",
}
...
def self_test() -> None:
    ...
    if stable_package_name(names[0]) != "smart-llmrouter-linux-amd64.tar.gz":
        raise AssertionError("binary stable name mismatch")
    if stable_package_name(names[2]) != "smart-llmrouter-docker-linux-amd64.tar.gz":
    ...
    mixed = names + [
        Path("smart-llmrouter-old-linux-amd64.tar.gz"),
        Path("smart-llmrouter-old-linux-arm64.tar.gz"),
    ]
