...
def main() -> int:
  ...
  with tempfile.TemporaryDirectory() as raw:
    ...
    need(report.returncode==0,report.stderr); rendered=(suite_logs/"evaluation-summary.md").read_text(); need("leak" not in rendered and "openai_chat" in rendered and "caller_output_cap_field" in rendered and "1 reported / 2 upstream attempts (0 tokens; 2 successful)" in rendered and "safe-provider" in rendered,"secret/raw content leaked or reasoning coverage missing")
