...
class FakeAWS:
    def __init__(self, *, root: bool = False, tag: bool = True, member: bool = False) -> None:
        self.root = root
        self.tag = tag
        self.member = member
    ...
    def __call__(self, profile: str, args: list[str]) -> dict[str, Any]:
        ...
        if args[:2] == ["sts", "get-caller-identity"]:
            return {"Account": ACCOUNT, "Arn": f"arn:aws:iam::{ACCOUNT}:root" if self.root else f"arn:aws:iam::{ACCOUNT}:role/platform-iac"}
...
def expect_error(fake: FakeAWS, *, principal: str = PRINCIPAL, apply: bool = False, confirmation: str = "") -> str:
    MODULE.run_aws = fake
    try:
...
def main() -> int:
    ...
    if "dedicated lifecycle IAM-user path" not in expect_error(FakeAWS(), principal=f"arn:aws:iam::{ACCOUNT}:user/unrelated"):
