...
// One-shot issuer for protected Fleet documents. Not packaged; run from repo root:
//   go run scripts/fleet_sign_docs.go <command> ...
package main
...
func main() {
	if len(os.Args) < 2 {
		die("usage: fleet_sign_docs <intent|admission|delete> ...")
	}
	switch os.Args[1] {
	case "intent":
