...
def make_package(root: Path, *, version: str = "a910827", arch: str = "amd64") -> Path:
    payload = root / f"smart-llmrouter-{version}-docker-linux-{arch}"
    ...
    write(payload / "images" / f"smart-llmrouter-{version}-linux-{arch}.tar", "image-bytes\n")
    archive = root / f"smart-llmrouter-{version}-docker-linux-{arch}.tar.gz"
...
class FakeRunner:
    ...
    def __call__(self, args, *, cwd=None) -> subprocess.CompletedProcess[str]:
        ...
        if "/app/bin/router-migrate" in argv:
            action = ""
            for part in argv:
        ...
        if argv[-2:] == ["up", "-d"]:
            ...
            return subprocess.CompletedProcess(argv, 0, stdout="", stderr="")
        if "ps" in argv:
            return subprocess.CompletedProcess(argv, 0, stdout="compose-router smart-llmrouter:a910827-linux-amd64 Up\n", stderr="")
        if "load" in argv:
