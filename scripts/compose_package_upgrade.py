...
PACKAGE_FILE_RE = re.compile(
    r"^smart-llmrouter-(?P<version>.+)-docker-linux-(?P<arch>amd64|arm64)\.tar\.gz$"
)
PACKAGE_DIR_RE = re.compile(
    r"^smart-llmrouter-(?P<version>.+)-docker-linux-(?P<arch>amd64|arm64)$"
)
IMAGE_TAR_RE = re.compile(r"^smart-llmrouter-.+-linux-(amd64|arm64)\.tar$")
...
def parse_package_filename(path: Path) -> tuple[str, str]:
    ...
    if not match:
        raise UpgradeError(
            f"package name {path.name} must match smart-llmrouter-<version>-docker-linux-<arch>.tar.gz"
        )
...
def inspect_package(package: Path) -> PackageInfo:
    ...
    return PackageInfo(
        ...
        arch=arch,
        top_dir=top,
        image_tag=f"smart-llmrouter:{version}-linux-{arch}",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    rollback.add_argument("--install-root", type=Path, default=Path("/opt/smart-llmrouter"))
