...
func main() {
	...
	if len(os.Args) < 2 {
		die("usage: metrum-genai-smartrouter-fleetctl <plan|deploy|status|delete|tenants|licenses|databases|smoke|customer> [flags]; plan/deploy/delete require --intent PATH")
	}
	switch os.Args[1] {
	case "status":
