...
- Planner workflow contract: workers create focused issue-linked PRs, monitor/action actionable PR/issue feedback, and create out-of-scope follow-up issues only with authority (otherwise escalate). Independent QA must confirm required checks, CODEOWNERS where applicable, no unresolved blocker, and rollback review. Never merge without human authorization of the exact PR head and named target; before issue closeout post the binding evidence comment. After merge, clean up only with explicit cleanup authority and proof the worktree/session is clean, unpushed, unleased, and not needed.
