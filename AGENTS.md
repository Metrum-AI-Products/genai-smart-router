...
- Crusoe Managed Inference:
  ...
  - The reference config catalogs Crusoe text models with source-dated pricing from Crusoe's pricing page and includes dedicated `crusoe-smoke` and `crusoe-gemma-smoke` groups. Do not add Crusoe to broad active model groups until the exact deployment account, model IDs, and Harbor workload outcomes pass; the current `big-coder` Crusoe Gemma target is tool-only for OpenAI Chat tool traffic and leaves Anthropic-compatible Gemma fallback routing intact.
