...
- WSH assignment identity: each project/runtime profile has exactly one authoritative WSH-hosted `sprint_planner` identity in a persisted named/tagged planner session. Do not launch or communicate with a duplicate in-process planner. Only that planner creates worker tmux windows and WSH sessions, and it binds each task to its protected profile, planner session, and exclusive worktree lease through bounded WSH CLI/REST metadata. Role names are policy contracts; the planner supplies bounded issue/role prompts using documented `codex [PROMPT]` or approved `codex exec [PROMPT]` forms, never `codex --agent`.
- Planner workflow contract: workers create focused issue-linked PRs, monitor/action actionable PR/issue feedback, and create out-of-scope follow-up issues only with authority (otherwise escalate as `MANAGER ATTENTION NEEDED`). Independent QA must confirm required checks, CODEOWNERS where applicable, no unresolved blocker, and rollback review. Never merge without human authorization of the exact PR head and named target; before issue closeout post the binding evidence comment. Workers do not remove or delete anything after merge. Only after GitHub confirms the authorized head merged into the named target may the planner, with separately recorded explicit cleanup authority, use the planner-controlled cleanup path for only that merged issue's leased worktree and local branch. Any ambiguity or failed clean/unpushed/unleased/not-needed proof leaves files and branches intact and is `MANAGER ATTENTION NEEDED`.
- V1 trust model: planner and workers sharing this UID are cooperative and mutually trusted. Profile/state 0600/0700 permissions are accidental-access hygiene only, not hostile-worker isolation; do not run untrusted agents or code under this identity or treat WSH/profile/session ownership as a security boundary. Hostile-worker isolation requires a separate authenticated OS or service identity.
