...
- Deployment/evaluation docs must treat each model group as a quality and cost contract. For every exposed group, document intended workloads, API shapes, modalities, tool dialects, success criteria, evaluation harness or objective test, cost/latency targets, promotion criteria, and rollback criteria. Emphasize that not every task needs the most expensive model; evaluation should preserve task outcomes while enabling lower-cost provider/model mixes where they pass.
- Public deployment evaluation docs should include security assessment, dependency/container scan expectations, diagnostics redaction, metrics-admin isolation, private-upstream network controls, client acceptance, model-group quality criteria, operational acceptance, rollback, and post-deploy cleanup.
