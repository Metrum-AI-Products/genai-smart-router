...
func setPublicDocsSecurityHeaders(w http.ResponseWriter) {
	...
	w.Header().Set("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; img-src 'self' data: https://storage.googleapis.com/eleven-public-cdn/; style-src 'self' 'unsafe-inline'; font-src 'self' data:; connect-src 'self' https://api.elevenlabs.io https://api.us.elevenlabs.io wss://api.elevenlabs.io wss://api.us.elevenlabs.io wss://livekit.rtc.elevenlabs.io; media-src 'self' blob:; worker-src 'self' blob:")
