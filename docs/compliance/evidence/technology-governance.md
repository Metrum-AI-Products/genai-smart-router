...
This record supports subissue **#967, Technology governance structure**, under parent issue **#961** for GenAI Smart Router. It records only repository-visible evidence of documented authority, role ownership, approval gates, and escalation paths. It does not establish that the documented controls operated in a production environment or that SOC 2 or ISO/IEC 27001 certification requirements have been met.
...
- The accepted ADR identifies `metrum-genai-smartrouter-fleetctl` as the sole Fleet lifecycle authority for deterministic plan, deploy, status, and separately approved delete operations. It explicitly distinguishes this authority from customer operations. **Update after 2026-09-01 collection:** Metrum engineering production authority is documented in `docs/EKS_PRODUCTION_OPERATIONS.md` (Fleet tenant `llm-api`); the ADR text no longer claims production remains rejected pending #518.
