...
In the reference deployment, active groups are deliberately limited to models that have passed live validation for their request shapes. General groups currently use Baseten GPT OSS 120B, MiniMax-M3, OpenRouter Gemma 4 26B Nitro capped at low ordinary-text weight, direct Moonshot Kimi K2.7 Code, low-weight Baseten Nemotron and GLM paths, Crusoe GLM 5.2 ordinary-text routing, and a 1% non-tool original OpenAI fallback. The code-heavy `big-coder` example uses MiniMax-M3 20%, direct Kimi K2.7 Code 17%, Baseten GPT OSS 120B 15%, Crusoe Nemotron 3 Nano Omni Reasoning 11% text-only, Fireworks GPT OSS 20B 11%, Fireworks Kimi K2.7 Code 5% text-only, Fireworks DeepSeek-V4-Flash 5%, Fireworks Qwen3.6 Plus 5% text-only, Baseten GLM 5.2 5%, Fireworks GLM 5.2 3%, Baseten Nemotron 2%, and OpenAI GPT-5.4 Nano 1% for ordinary text. Crusoe Gemma 4 31B-it is historical/catalog/smoke-only unless a deployment revalidates the exact route. Tool-bearing Codex, OpenAI Chat tool, and Claude Code traffic use separate tool-validated targets, with MiniMax M3 preferred for OpenAI Responses and Anthropic-compatible tool routes, Fireworks Responses Kimi available at low tool-only weight, and OpenRouter fallbacks retained without active Sonnet-heavy tool weighting in `big-coder`.
