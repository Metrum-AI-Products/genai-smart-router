...
| Metrum customer control plane / commercial fulfillment | Optional in monorepo | Fleet, commerce, licensing, and customer-lifecycle packages remain in this Apache-2.0 tree as optional operator tooling. They are not required to run the Community router. Customer Docker images omit Fleet/commerce CLIs. Payment processing, card data, and private signing keys stay out of the request path; the router is not a billing ledger. |
