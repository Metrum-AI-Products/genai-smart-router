...
LRP estimates calibrated quality standard deviation across members. If the
would-be primary exceeds `uncertainty_threshold` (reviewed default `0.15`),
selection abstains to `abstention_anchor` (or the first fallback) and emits the
bounded label `lrp:uncertain`.
...
`exploration_projects` (approved calibration traffic). When an explore draw
fires, LRP samples `q ~ Normal(mean, std)` from ensemble uncertainty and runs
the same cheapest-above-floor policy, labeling `lrp:explore-thompson`.
...
cosine distance of new embeddings to the training centroid. Outputs are bounded
...
- **Synthetic wiring:** unit tests under
  ...
  cover abstention labels, Thompson vs epsilon rates, PSI/embedding drift,
  auto-shadow recommendation, and cold-start seed/injection.
