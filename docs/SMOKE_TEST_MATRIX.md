...
| `accounts/fireworks/models/deepseek-v4-flash` | Accepted but tiny text budget returned incomplete text | Passed | Passed | Passed | Returned one call with incomplete status | Honored cap with incomplete status | Active in production/reference `big-coder` ordinary-text routing at 40% as of 2026-06-30; keep image and untested skins disabled until separately validated |
