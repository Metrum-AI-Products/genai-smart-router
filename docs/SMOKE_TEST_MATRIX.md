...
| Chat-to-Responses bridge | restricted smoke group with `bridges.chat_to_responses.enabled: true`; Chat non-streaming text request, Chat function-tool request when `tools: true`, optional two-request stateful-session proof when `stateful_sessions.enabled`, negative unsupported shape such as `stream:true`, usage rows with inbound Chat and target Responses, and safe translation-shape diagnostics |
