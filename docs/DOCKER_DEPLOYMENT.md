...
The router image also embeds authenticated admin report assets, including the Metrum-branded browser shell, local logo/font assets, JavaScript, and chart bundle. They are disabled by default and served only under `/admin/reports/` after `server.admin_reports.enabled: true`, Basic Auth, and Casbin `admin:reports` policy are configured. Public `/docs/` remains separate from report data.
...
For browser report rollout, smoke `/admin/reports/` and `/admin/reports/api/summary?since=24h` with an authorized browser-admin user, verify the Metrum-branded shell loads, confirm the dark/light toggle persists after reload, confirm ordinary router tokens receive `403 reports-forbidden`, and disable reports by setting `server.admin_reports.enabled: false` if rollback is needed.
