...
running GenAI Smart Router in front of in-cluster vLLM/ROCm on AMD Instinct.
Client applications never talk to vLLM directly in the intended workflow. They
call Smart Router with a **router model-group name** and a **router caller
token**.
...
| Smart Router | Auth (SHA-256 of caller token), allow-list, static routing to in-cluster Service DNS, license check, usage SQLite, `/readyz`. **No** `amd.com/gpu` request. |
...
## 7. Install Smart Router (Helm + license)
