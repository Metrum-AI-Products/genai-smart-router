...
GenAI Smart Router can route to enterprise-owned inference services that expose OpenAI-compatible APIs, including vLLM and SGLang. Keep these services on private network names, expose the router as the governed ingress point, and use router model groups as the caller-facing contract.
