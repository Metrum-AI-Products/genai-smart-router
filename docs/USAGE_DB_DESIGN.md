...
Draft rollups for the same exact type/window may be regenerated idempotently. Finalized rollup windows are immutable through the rollup generator, and new rollup runs are rejected if their type/window overlaps an existing finalized window. Raw request purge is not part of this rollup foundation. The retention foundation uses finalized daily rollup metadata as a prerequisite before any future `usage_detail` delete can be considered.
