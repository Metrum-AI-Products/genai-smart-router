...
| Router configuration | `docs-site/docs/configuration/router-config.md` | `config.example.yaml`, `README.md`, feature-specific runbooks in `docs/` | YAML fields, defaults, validation behavior, rollout and rollback notes |
| Dynamic score routing | `docs-site/docs/configuration/dynamic-score-routing.md` | `docs/DYNAMIC_SCORE_ROUTING.md`, `config.example.yaml`, routing tests | Strategy behavior, safe inputs, evaluation metadata, diagnostics, rollback |
...
| Self-hosted upstreams | `docs-site/docs/configuration/self-hosted-upstreams.md` | `docs/SELF_HOSTED_UPSTREAMS.md`, `config.example.yaml`, smoke scripts | vLLM/SGLang-style setup, served model IDs, tool parser validation, private URL handling |
| VLM and image routing | `docs-site/docs/configuration/image-analysis-vlm.md` | `docs/SMOKE_TEST_MATRIX.md`, `config.example.yaml`, provider smoke records | Modalities, exact model/dialect validation, image token/cost fields, fallback behavior |
| API compatibility and errors | `docs-site/docs/reference/*.md` | `README.md`, router handlers, tests, `docs/SMOKE_TEST_MATRIX.md` | OpenAI/Anthropic compatibility, structured error types, auth and quota semantics |
| Usage reporting and admin reports | `docs-site/docs/operations/usage-reporting.md`, `docs-site/docs/operations/admin-browser-reports.md`, `docs-site/docs/operations/report-examples.mdx` | `docs/USAGE_REPORTING_PLAYBOOK.md`, `docs/USAGE_DB_DESIGN.md`, `docs/SECURITY_REVIEW_NOTES.md` | Report fields, scalar usage schema, authorization, redaction, retention, customer-safe examples |
| License-protected deployments | `docs-site/docs/operations/license-protected-deployments.md` | `docs/LICENSE_OPERATIONS.md`, license code/tests, release packaging checks | Customer install/renewal/status behavior publicly; signing and bypass operations internally |
