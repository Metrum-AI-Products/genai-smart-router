...
This document is a detailed, renderable architecture for Metrum Smart Router in
...
primary source names Metrum Smart Router. Placing Metrum Smart Router as the
...
| **Repo-supported** | Implemented in Metrum Smart Router / packaging in this repository, but **not** proven by the AMD #954 live matrix. |
...
|---|---|---|
| On-prem k3s + Metrum Smart Router Helm + license wrapper | Validated (PR #954) | Manual overlay; NVIDIA blueprint used only as config/chart factory |
...
| OpenTelemetry Collector ingest of Prometheus + OTLP | Proposed | Metrum Smart Router has **no** native OTLP exporter today |
| End-to-end `traceparent` through Metrum Smart Router | Proposed gap | Correlate via `X-Request-Id` + usage diagnostics |
...
- Enterprise self-hosted / on-prem pattern: Metrum Smart Router as the governed API edge.
...
- Claiming Metrum Smart Router schedules GPUs, owns ROCm drivers, or performs KV-aware replica pick.
...
|---|---|---|
| Metrum Smart Router | Auth, allow lists, model-group policy, request-shape eligibility, usage, `/metrics` isolation | GPU scheduling, ROCm drivers, replica KV pick |
...
Metrum Smart Router `config.yaml` (generated via NVIDIA local-serving blueprint as a
...
disaggregation, add llm-d **behind** Metrum Smart Router:

| Responsibility | Owner |
|---|---|
| Caller auth, allow list, model-group contract, enterprise policy | Metrum Smart Router |
| Select OpenAI-compatible pool URL for the group | Metrum Smart Router (static or weighted to `llm-d-…` Service) |
...
for the same hop. Metrum Smart Router exposes **deployment-defined group names** to
...
## 7. Metrum Smart Router routing policies and agentic workloads
...
### 9.2 Metrum Smart Router Prometheus families (repo-supported)
...
metrics for HPA/KEDA. Do not scale from Metrum Smart Router `/metrics` series that carry
...
| Metrum Smart Router native OTLP spans / `traceparent` propagation | **Not implemented** (repo search empty) |
...
| Metrum Smart Router replicas | SQLite usage → single writer / Recreate; Postgres for multi-replica | Repo-supported |
...
4. Install Metrum Smart Router via `scripts/helm_install_with_license.sh`; assert **no**
...
| Surface | Direct upstream | Through Metrum Smart Router | AMD #954 |
|---|---|---|---|
...
5. Never claim OTel end-to-end tracing through Metrum Smart Router until the code
