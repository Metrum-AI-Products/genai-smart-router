...
Recovery is owner-profile bound and refuses a different profile, a legacy/ambiguous drain, or any shared worker lease. It repeats exact planner WSH and tmux cleanup, then clears the drain only after a fresh bounded result proves the named planner session is absent and the exact tmux control session is absent. The sole exception is an expected `unavailable` result after the launcher has durably recorded the exact owner/profile/generation/planner-session absence proof before stopping the WSH server; duplicate, wrong, missing, or otherwise uncertain identities still retain the drain. Never manually clear the drain, delete the registry, or infer that a same-UID process is harmless. This remains a cooperative same-host/shared-filesystem procedure, not cross-host coordination or hostile-worker containment.

## Worker and QA instructions

Workers receive only the planner-provisioned session/worktree assignment. They must not create, attach, inspect, or steer tmux/WSH sessions and must not use WSH MCP. They work only in their assigned external worktree and report progress, evidence, blockers, and every required decision to the sprint planner as `MANAGER ATTENTION NEEDED`.
