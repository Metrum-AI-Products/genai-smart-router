...
The profile supplies all host-specific values: `profile_id`, logical `planner_owner`, tmux session name, the required opaque `wsh_server_identity`, `bootstrap_identity_wait_seconds` (an integer from 1 through 10), local state directory, base ref, lease TTL, forbidden WSH environment markers, and the approved command templates. The launcher derives the Git repository from the current directory and discovers registered worktrees through `git worktree list`; it does not contain a host, port, socket, checkout path, URL, or token.
...
`bootstrap` starts or reuses the local `wsh-server` window, then uses a single monotonic, profile-bounded readiness interval only for an `unavailable` authoritative identity observation. It polls only that same identity command and creates the `sprint-planner` session only after an exact match; missing, mismatch, malformed, ambiguous, or expiry fail closed immediately or at the deadline without cleanup. It revalidates immediately before planner-session creation. `attach` and every other lifecycle path require one fresh identity match with no readiness wait. Detaching and later attaching does not recreate WSH sessions or discard planner metadata.
...
Recovery is owner-profile bound and refuses a different profile, a legacy/ambiguous drain, or any shared worker lease. It normally repeats exact planner WSH and tmux cleanup only after a fresh bounded identity/list/tag path. The sole exception is `recover-shutdown --cleanup-authorized` after the launcher has durably recorded the exact identity-bound planner-absence proof: the registry and non-legacy owner drain must have the exact current generation, no shared or local worker leases, and an exact proof schema binding profile, owner, planner-session ID, generation, a timezone-aware timestamp, and the identity-bound proof kind. The exact `wsh-server` window must be absent on entry and immediately before drain clearance, with no unexpected control windows. This branch makes **zero WSH calls** and completes only residual local tmux/state/drain cleanup. “Stale” means a changed profile, owner, session, generation, timestamp, or schema—not elapsed time. Missing, extra, malformed, legacy, corrupt, contradictory, or reappearing-window evidence retains the drain; every other recovery remains freshly identity-bound. Audit records only safe `shutdown-recovery` action/outcome/disposition classes. Never manually clear the drain, delete the registry, or infer that a same-UID process is harmless. This remains a cooperative same-host/shared-filesystem procedure, not cross-host coordination or hostile-worker containment.
