...
`bootstrap` starts or reuses the named tmux session and creates missing `wsh-server` and `sprint-planner` windows without duplicating either. `attach` only attaches to that existing named session. Detaching and later attaching does not recreate WSH sessions or discard planner metadata.

Do not assume any prior WSH server is running. `preflight` first uses the bounded list surface to prove exactly one planner session name, then runs the installed exact-session read-only `wsh -L <profile> tag <planner-session>` query. It parses only the tag-value substring after WSH's exact `Session '<planner-session>':` delimiter and requires `planner-<profile>` as a complete tag token; a matching session name never authorizes itself. It also runs the bounded `codex --help` acceptance check without launching a worker task. Before any worker lease allocation, `launch-worker` repeats that fresh planner identity check and rejects an absent, duplicate, or wrongly tagged planner identity. A failed preflight never creates a worker window or writer lease. An unavailable server is a graceful `status-unavailable` condition until bootstrap, not a reason to reuse or restart an unrelated server.
...
Workers receive only the planner-provisioned session/worktree assignment. They must not create, attach, inspect, or steer tmux/WSH sessions and must not use WSH MCP. They work only in their assigned external worktree and report progress, evidence, blockers, and human-needed decisions to the sprint planner.

The planner-issued effective prompt requires a focused issue-linked PR; monitored and actioned PR/issue feedback; authority-gated out-of-scope follow-ups (otherwise escalation); independent QA; required checks, CODEOWNERS where applicable, no unresolved blocker, and rollback review. Merge requires explicit human authorization of the exact PR head and named target. Before issue closeout, the planner records a binding issue comment with PR, head, checks, QA, rollback, and merge-authority evidence. After merge, cleanup needs explicit authority plus proof that the worktree/session is clean, unpushed, unleased, and not needed.
