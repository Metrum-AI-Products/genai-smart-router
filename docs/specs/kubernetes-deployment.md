...
| GenAI Smart Router | Authenticates the caller, enforces caller limits, selects a model group and target, records usage, and calls the selected upstream. | It does not schedule GPUs, tune replica topology, or select a replica inside an inference pool. |
...
This boundary makes llm-d complementary. Smart Router selects the target pool. llm-d
...
llm-d project documents. Do not add a Smart Router controller for llm-d.
...
perform profiling-driven topology tuning. Smart Router MUST NOT duplicate that tuning.
...
contention design. Kueue does not admit router HTTP requests. Smart Router continues to
