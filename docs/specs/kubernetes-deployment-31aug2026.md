...
flowchart LR
    ...
    Semantic --> Router[GenAI Smart Router]
...
| GenAI Smart Router | Authenticates the caller, enforces caller limits, selects a model group and target, records usage, and calls the selected upstream. | It does not schedule GPUs, tune replica topology, select a replica inside an inference pool, or own node drivers. |
...
This boundary makes llm-d complementary. Smart Router selects the target pool or
...
serving-stack concern, not a Smart Router CRD.
...
Mooncake has no first-party CRD in current upstream docs. Do not invent a Smart Router
Mooncake CRD. Do not treat OME `KVCachePool` as a Smart Router resource.

llm-d MAY use Mooncake Store as a vLLM KV offload tier and `MooncakeConnector` for
prefill/decode transfer. Smart Router still selects the model group and target URL or
...
documented overlays as the llm-d project documents. Do not add a Smart Router controller
for llm-d. llm-d is a CNCF Sandbox project and remains pre-1.0.

Smart Router selects the model group and pool. llm-d selects the replica within the
...
profiling API. DGDR already performs profiling-driven topology tuning. Smart Router
...
Smart Router treats the exposed endpoint as a normal upstream after the configuration
is written.
...
admit router HTTP requests. Smart Router continues to enforce `callers[].rate`,
...
security threat model. It is outside router architecture. It is not the Smart Router
...
| Disaggregated serving | llm-d `v0.9+` | Helm charts and overlays, not a Smart Router operator. CNCF Sandbox. |
