...
For aggregate triage, run `router-usage-report --traffic-shaped-only --since 24h` and inspect Traffic Shaping By User / Project, By Key, By Client, and By Model Group. In the browser admin UI, use the Traffic shaping overview, Shaping users, Shaping keys, Shaping clients, and Shaping groups tabs.
