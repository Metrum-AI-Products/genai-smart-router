...
Admin reports and `router-usage-report --traffic-tuning-advisor` can turn those safe shaping, upstream attempt, fallback, latency, and cancellation fields into conservative operator recommendations such as enabling queueing, increasing a specific burst/depth, slowing a client, tuning provider shared capacity, or routing around an incompatible upstream target. The advisor is report-only and never modifies config.
...
big-coder  Temporary code-heavy route: Fireworks DeepSeek-V4-Flash 40%, MiniMax M3 20%, Kimi K2.7 Code 15%, Crusoe GLM 5.2 15%, OpenAI GPT-5.4 Nano 10% for ordinary text and OpenAI Chat tool traffic where supported; Fireworks Responses Kimi K2.7 Code remains available for Codex/Responses tool traffic, while MiniMax and Kimi Anthropic-compatible targets remain available for Claude Code-style tool traffic.
```
