...
- Fireworks AI is configured as an external hosted OpenAI-compatible `openai-chat` provider with `base_url: https://api.fireworks.ai/inference/v1`, `api_key_env: FIREWORKS_API_KEY`, bearer auth, and an explicit `User-Agent` header. Fireworks `accounts/fireworks/models/gpt-oss-20b` passed direct text, streaming, `max_tokens: 1`, `reasoning_effort` low/medium/high, OpenAI Chat auto tools with `max_tokens >= 256`, forced `tool_choice`, and JSON schema structured-output smokes on 2026-06-27, even though it was not listed by `/models` for the validated account. Fireworks Serverless pricing lists GPT OSS 20B at $0.07/M input, $0.035/M cached input, and $0.30/M output. Keep other Fireworks models catalog-only until the exact account, model ID, entitlement, pricing, cap behavior, router-level smokes, and workload gates pass. Do not claim Fireworks OpenAI Responses, Anthropic Messages, image, video, or audio support until those exact skins pass separately.
...
Fireworks AI is an external hosted OpenAI-compatible provider. Fireworks documentation checked on 2026-06-27 lists `https://api.fireworks.ai/inference/v1` as the OpenAI-compatible base URL, `FIREWORKS_API_KEY` authentication, account-qualified model IDs, and Serverless pricing where GPT OSS 20B is $0.07/M input, $0.035/M cached input, and $0.30/M output. A direct `/models` check on 2026-06-27 returned account-visible IDs such as `accounts/fireworks/models/gpt-oss-120b`, `accounts/fireworks/models/deepseek-v4-pro`, `accounts/fireworks/models/kimi-k2p6`, and `accounts/fireworks/models/glm-5p2`; `accounts/fireworks/models/gpt-oss-20b` was not listed but completed successfully. Direct Fireworks `accounts/fireworks/models/gpt-oss-20b` OpenAI Chat text, streaming, `max_tokens: 1`, `reasoning_effort` low/medium/high, auto tool with `max_tokens >= 256`, forced `tool_choice`, and JSON schema structured-output smokes passed with an explicit `User-Agent`. Fireworks returns `reasoning_content` alongside visible content, so usage and acceptance checks should use realistic budgets and inspect both final content and token totals. Router-level and Harbor workload validation are required for production rollout, and Fireworks OpenAI Responses, Anthropic Messages, image, video, and audio remain disabled until separately validated.
