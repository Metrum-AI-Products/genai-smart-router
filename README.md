...
Diagnostics are written alongside usage when `server.diagnostics.enabled` is true. Each request can have child rows in `request_attempts`, `request_trace_events`, `request_traffic_shape_events`, `request_upstream_shape_events`, `request_upstream_error_details`, and `request_errors`, all keyed by `request_id`. Use the `X-Request-Id` response header or the `request_id` in an error body to join these rows during incident response. Diagnostic rows store provider/model/status/timing/error-class data plus safe caller and upstream shaping bucket decisions, not raw prompts, images, bearer tokens, provider keys, token hashes, full upstream headers, or raw upstream response bodies. `store_sanitized_upstream_errors` can keep bounded allowlisted provider error fields such as `code`, `type`, `param`, `request_id`, and sanitized `message` in scalar `request_upstream_error_details` rows for troubleshooting. It is not content capture: arbitrary upstream bodies are collapsed to a redaction marker, and prompt-like fields, nested upstream bodies, and secret-shaped values are redacted before JSONL or usage DB persistence.
...
big-coder  Code-heavy route: MiniMax-M3 20%, direct Kimi 17%, Baseten GPT OSS 120B 15%, Crusoe Nemotron 3 Nano Omni Reasoning 11% text-only, Fireworks GPT OSS 20B 11%, Fireworks Kimi K2.7 Code 5% text-only, Fireworks DeepSeek-V4-Flash 5%, Fireworks Qwen3.6 Plus 5% text-only, Baseten GLM 5%, Fireworks GLM 5.2 3%, Baseten Nemotron 2%, OpenAI GPT-5.4 Nano 1% non-tool, plus separately validated tool-only OpenAI Chat/Responses/Anthropic-compatible fallbacks including low-weight Fireworks Responses Kimi.
```
