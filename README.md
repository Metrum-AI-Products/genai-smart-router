...
When decision telemetry is enabled, usage/admin reports expose safe dynamic-score and admission buckets for operations: enabled signal names, score/value/final-score buckets, threshold/filter buckets, max-token cap filtering, max-token buckets, input-token buckets, and quota/admission reasons. Daily rollups preserve those buckets in normalized scalar rows so commercial reports can outlive raw request-level detail retention.
