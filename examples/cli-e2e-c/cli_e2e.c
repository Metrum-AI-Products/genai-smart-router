...
static int test_codex(const char *expect) {
    ...
    char *q_provider_cfg = shell_quote("model_provider=\"metrum-ai-router\"");
    char base_cfg[700];
    snprintf(base_cfg, sizeof(base_cfg), "model_providers.metrum-ai-router.base_url=\"%s\"", base_v1);
    char *q_base_cfg = shell_quote(base_cfg);
    char *q_name_cfg = shell_quote("model_providers.metrum-ai-router.name=\"Metrum AI Router\"");
    char *q_key_cfg = shell_quote("model_providers.metrum-ai-router.env_key=\"METRUM_ROUTER_KEY\"");
    char wire_cfg[128];
    snprintf(wire_cfg, sizeof(wire_cfg), "model_providers.metrum-ai-router.wire_api=\"%s\"", wire_api);
